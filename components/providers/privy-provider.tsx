    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      ...
      <Card className="relative z-10 w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription>
            To use this template, you need a Privy App ID. Follow the steps
        ...
        <CardContent className="space-y-6">
          <ol className="space-y-3 text-sm text-muted-foreground">
            ...
            <li className="flex gap-3">
              ...
              <span>
                Create a <code className="rounded bg-muted px-1.5 py-0.5 text-xs font-mono">.env.local</code> file in the project root
              </span>
            </li>
            <li className="flex gap-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">
...
export function PrivyAuthProvider({
  ...
  if (!isValidAppId(PRIVY_APP_ID)) {
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        '[privy-auth] NEXT_PUBLIC_PRIVY_APP_ID is missing or set to a placeholder value.'
      );
    }
